...
use piet::kurbo::{Circle, Point, Rect, RoundedRect, Vec2};

use piet::{Color, Error, FixedGradient, FixedLinearGradient, FixedRadialGradient, GradientStop, RenderContext, StrokeStyle, Text, FontBuilder, TextLayoutBuilder};
...
pub fn draw<R: RenderContext>(rc: &mut R) -> Result<(), Error> {
    ...
    stroke_style.set_line_join(piet::LineJoin::Round);
    ...
    rc.stroke(Circle::new(Point::new(30.0, 20.0), 15.0), &radial_gradient, 5.0);
    rc.stroke_styled(Circle::new(Point::new(30.0, 60.0), 15.0), &radial_gradient, 5.0, &stroke_style);
    ...
    rc.stroke_styled(RoundedRect::new(60.0, 0.0, 100.0, 30.0, 7.0), &linear_gradient, 5.0, &stroke_style);
    stroke_style.set_line_cap(piet::LineCap::Square);
    rc.stroke_styled(RoundedRect::new(60.0, 40.0, 100.0, 70.0, 3.0), &linear_gradient, 5.0, &stroke_style);
    ...
    let jp_text = rc.text().new_text_layout(&jp_font, "ローカリゼーション作品", None).build()?;
    let font = rc.text().new_font_by_name("Segoe UI", 8.0).build()?;
    let en_text = rc.text().new_text_layout(&font, "Text with gradient", None).build()?;
